/*
    ...
    QQ交流群：184075694 （优先发布RenewTheme及内测包）
*/

jQuery(function($){jQuery.ajaxSetup({cache:false});});/*重置ajax并Cancel缓存*/

/*初始化语言*/
if ($.fn.pagination){
	$.fn.pagination.defaults.beforePageText = 'No.';
	$.fn.pagination.defaults.afterPageText = 'Total {pages} pages';
	$.fn.pagination.defaults.displayMsg = 'Showing {from} to {to}, total {total} records';
}
if ($.fn.datagrid){
...
if ($.fn.validatebox){
	...
	$.fn.validatebox.defaults.rules.length.message = 'The input content length must be between {0} and {1}';
	$.fn.validatebox.defaults.rules.remote.message = 'Please correct this field';
}
if ($.fn.calendar){
	$.fn.calendar.defaults.weeks = ['Mon', 'Tues', 'Wed', 'Thurs', 'Fri', 'Sat', 'Sun'];
...
;(function($){
	$.insdep={
		ajax:function(c){
			var d={
				url:"",
				async:false,//true异步请求,false同步请求,注：同步请求将锁住浏览器，用户其它Operate必须等待请求完成才可以执行
				cache:false,
				type:"POST",
				dataType:"html",
				beforeSend:function(){
