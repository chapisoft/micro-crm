...
{
  public class ContactActivitiesController : Controller
  {
    private readonly IMediator mediator;
    private readonly IContactActivityService contactActivityService;
    ...
    public ContactActivitiesController(
          ...
          ILogger<ContactActivitiesController> logger)
    {
    ...
    public async Task<JsonResult> GetData(ActivityPaginationQuery request)
    {
      try
      {
        //var filters = PredicateBuilder.FromFilter<Activity>(filterRules);
        //var total = await this.contactActivityService
        //                     .Query(filters).CountAsync();
        //var pagerows = (await this.contactActivityService
        //                     .Query(filters)
    ...
    //public async Task<IActionResult> AddOrEdit(Activity request)
    public async Task<IActionResult> AddOrEdit(CreateOrEditActivityCommand request)
    {
      ...
      //  if (request.Id == 0)
      //  {
      //    this.contactActivityService.Insert(request);
      ...
      //    try
      //    {
      //      this.contactActivityService.Update(request);
      //      await this.unitOfWork.SaveChangesAsync();
      //    }
      //    catch (DbUpdateConcurrencyException)
      //    {
      //      if (!await this.contactActivityService.ExistsAsync(request.Id))
      //      { return NotFound(); }
      //      else
      //      { throw; }
      //    }
      ...
      //GET: ContactActivities/Delete/:id
      [HttpGet]
    public async Task<JsonResult> Delete(int id)
    {
      try
      {
        await this.contactActivityService.DeleteAsync(id);
        await this.unitOfWork.SaveChangesAsync();
        return Json(new { success = true });
      }
    ...
    public async Task<JsonResult> DeleteChecked(DeleteActivityCommand request)
    {
      //try
      //{
      //  foreach (var key in id)
      //  {
      //    await this.contactActivityService.DeleteAsync(key);
      //  }
      //  await this.unitOfWork.SaveChangesAsync();
      //  return Json(new { success = true });
      //}
    ...
    [HttpPost]
    public async Task<JsonResult> AcceptChanges(ContactActivity[] activities)
    {
      if (ModelState.IsValid)
      {
        try
        {
          foreach (var item in activities)
          {
            ...
            this.contactActivityService.ApplyChanges(item);
          }
    ...
    {
      var fileName = "activities_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xlsx";
      var filters = PredicateBuilder.FromFilter<ContactActivity>(filterRules);
      var stream = await this.contactActivityService.Export(filters, sort, order);
        ...
        foreach (var formFile in uploadfiles)
        {
          if (formFile.Length > 0)
          {
            ...
            stream.Seek(0, SeekOrigin.Begin);
                  await this.contactActivityService.ImportData(stream);
