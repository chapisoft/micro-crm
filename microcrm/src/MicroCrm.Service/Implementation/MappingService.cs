...
namespace MicroCrm.Service
{
  public class DataTableImportMappingService : Service<DataTableImportMapping>, IDataTableImportMappingService
  {
      ...
      => await Task.Run(() =>
                        {
                          ...
                          var asm = Assembly.Load("MicroCrm.Domain");
                          list = asm.GetTypes()
    ...
    public async Task ImportDataTableAsync(DataTable datatable)
    {
      ...
      {
        throw new KeyNotFoundException("没有找到DataTableImportMapping对象的ExcelImport ConfigMessage，请执行[System Management/ExcelImport Config]");
      }
